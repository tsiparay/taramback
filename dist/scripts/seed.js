...
const seedPath = process.env.SEED_PATH ?? path_1.default.join(process.cwd(), 'seed', 'seed.json');
...
async function main() {
    ...
    try {
        ...
        await run(db, `CREATE TABLE IF NOT EXISTS networks (
        ...
        await run(db, `CREATE TABLE IF NOT EXISTS article_categories (
        ...
        for (const a of seed.articles) {
            await run(db, 'INSERT INTO articles (id, title, content, status, featured, publishedAt, categoryId, networkId, authorId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [
                a.id,
                a.title,
                a.content,
                a.status,
        ...
        for (const no of seed.notifications) {
            await run(db, 'INSERT INTO notifications (id, userId, articleId, type, recipientsJson, subject, status, error, sentAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [
                no.id,
                no.userId,
                no.articleId,
                no.type,
